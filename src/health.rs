...
fn parse_ext_csd(
    ...
    EmmcHealthSnapshot {
        ...
        cmdq_support: Some(decoded.cmdq_support),
        cmdq_depth: Some(decoded.cmdq_depth),
...
pub fn parse_cid_hex(raw_hex: &str) -> Result<CidInfo> {
    ...
    let prv_major = raw[9] >> 4;
    let prv_minor = raw[9] & 0x0f;
    ...
    Ok(CidInfo {
        ...
        oid: raw[1..3]
            ...
            .join(""),
        pnm,
        prv: format!("{prv_major}.{prv_minor}"),
        psn: format!("0x{psn:08X}"),
        mdt: format_manufacturing_date(month, year),
        cbx: Some((raw[1] >> 6) & 0x03),
        prv_major: Some(prv_major),
        prv_minor: Some(prv_minor),
        psn_numeric: Some(psn),
        mdt_year: Some(year),
        mdt_month: Some(month),
        crc: Some(raw[15] >> 1),
