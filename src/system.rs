...
pub fn doctor(paths: &AppPaths) -> DoctorReport {
    ...
    let geometry_count = devices
        ...
        .count();
    checks.push(if geometry_count > 0 {
        pass_check(
            "block_geometry",
        ...
        warn_check(
            "block_geometry",
            "no queue geometry could be read from /sys/class/block",
        )
    });
    let discard_count = devices
        ...
        .count();
    checks.push(if discard_count > 0 {
        pass_check(
            "discard_support",
        ...
        warn_check(
            "discard_support",
            "no visible device advertises discard support",
        )
    });
